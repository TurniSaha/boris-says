import { aliasToId, type ModelAlias } from './models.js';
...
export function createAnthropicBackend(
  ...
  return {
    ...
    async complete({ system, user, maxTokens, model }: LlmCompleteOptions): Promise<string | null> {
      try {
        if (!apiKey) return null;
        const id = aliasToId[model as ModelAlias];
        if (!id) return null;
