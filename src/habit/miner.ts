/**
 * src/habit/miner.ts — the throttled cross-session habit miner (SPEC §7.2, §5.5.6).
 *
 * THROTTLE (decision #7, §7.2): mine ONLY when BOTH
 ...
 * representative `match_phrases`). Defensive parse mirroring `parseJudgeVerdict`.
 * A null/unconfigured backend is a no-op.
 *
 * GUARDRAILS (§5.5.6a structural drops — never trust the prompt alone):
 *   - < 3 DISTINCT sessionIds  -> drop
 *   - empty `fix`              -> drop
 *   - empty `why_inefficient`  -> drop
 *
 * DISMISSAL-SIMILARITY GATE (§5.5.6b): before upserting a NEW-keyed open pattern,
 * Jaccard over its anchor-token signature vs every DISMISSED pattern's signature;
 * overlap >= DISMISSAL_JACCARD_THRESHOLD (0.6) -> DROP (treat as the dismissed
 * behavior, never reopen). The surviving pattern persists its `anchorSignature`.
 *
 * SELF-MATCH CALIBRATION (§5.5.6c): run each occurrence's evidence through the §7.4
 * matcher against the pattern's stored phrases; if fewer than 3 historical
 * occurrences self-match, REJECT the pattern (the phrases do not generalize).
 *
 * The miner does NOT own state writes beyond RETURNING the advanced state (caller
 * deposits it — matches the cascade caller-deposits pattern).
 */
import type { LlmBackend } from '../llm/backend.js';
import type { CoachState } from '../state/store.js';
import type { CorpusPrompt } from '../jsonl/corpus-reader.js';
import { parseMinerPatterns, type MinedPattern } from './miner-parse.js';
import { type PatternsStore, type Pattern } from './patterns-store.js';
...
export const MINER_SYSTEM =
  'You analyze a developer\'s recent TYPED prompts for RECURRING HABITS worth coaching. ' +
...
/** What the miner returns; the caller persists `nextState` and surfaces nothing else. */
export interface MinerResult {
  /** True iff a mine actually ran (throttle passed AND backend produced output). */
  readonly mined: boolean;
  /** Why a mine did not run (for observability); null when it did. */
  readonly skippedReason: 'throttle_events' | 'throttle_cooldown' | 'no_backend' | null;
  /** Patterns upserted this run (post-guardrail survivors). */
  readonly upserted: readonly Pattern[];
  /** The advanced state the caller MUST persist (watermark + lastMinedAt). */
...
export interface MinerInput {
  ...
  /** The typed-prompt corpus (already read via corpus-reader), OR a reader fn. */
  readonly corpus: readonly CorpusPrompt[] | (() => readonly CorpusPrompt[]);
  readonly store: PatternsStore;
  readonly now: number;
}

/** Resolve the corpus whether passed as an array or a lazy reader fn. */
function resolveCorpus(corpus: MinerInput['corpus']): readonly CorpusPrompt[] {
  return typeof corpus === 'function' ? corpus() : corpus;
}

/** Render the typed-prompt slice with session boundaries for the model. */
function renderCorpus(corpus: readonly CorpusPrompt[]): string {
  return corpus
...
function signatureOf(phrases: readonly string[]): Set<string> {
  const sig = new Set<string>();
  for (const phrase of phrases) for (const tok of anchorTokens(phrase)) sig.add(tok);
  return sig;
}

/** Count DISTINCT sessionIds across a mined pattern's occurrences. */
function distinctSessions(p: MinedPattern): number {
 ...
 */
export function temporallySeparatedSessionCount(
  p: MinedPattern,
  ...
  const repBySession = new Map<string, number>();
  for (const o of p.occurrences) {
    if (o.ts === 0) continue; // unknown time: does not anchor a window.
    const prev = repBySession.get(o.sessionId);
    if (prev === undefined || o.ts < prev) repBySession.set(o.sessionId, o.ts);
  }
 ...
 * phrases.
 */
function selfMatchCount(p: MinedPattern): number {
  const probe: Pattern = toPattern(p, 'open', 0);
  let n = 0;
  for (const occ of p.occurrences) {
    if (matchHabit(occ.evidence, [probe]) !== null) n += 1;
  }
  return n;
}

/** Build a full persisted Pattern from a mined one. */
function toPattern(p: MinedPattern, status: 'open', createdAt: number): Pattern {
  return {
 ...
 */
export async function runHabitMiner(input: MinerInput): Promise<MinerResult> {
  ...
  const structurallyValid = mined.filter(
    (p) =>
      distinctSessions(p) >= MIN_DISTINCT_SESSIONS &&
  ...
  const calibrated = structurallyValid.filter(
    (p) => selfMatchCount(p) >= MIN_DISTINCT_SESSIONS,
  );
  ...
  for (const p of calibrated) {
    ...
    if (isNewKey) {
      ...
      if (tooSimilar) continue; // treat as the dismissed behavior — drop, never reopen.
    }
 ...
 */
async function draftSurvivors(
  survivors: readonly Pattern[],
  priorByKey: ReadonlyMap<string, Pattern>,
  backend: LlmBackend,
  now: number,
): Promise<Pattern[]> {
  const out: Pattern[] = [];
...
function noop(state: CoachState, reason: MinerResult['skippedReason']): MinerResult {
