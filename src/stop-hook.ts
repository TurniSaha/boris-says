...
import { createStore, type Store, type Tip } from './state/store.js';
import {
  resolveBaseDir,
  isEnabled,
  projectKeyForCwd,
...
/** The raw Stop stdin payload (extra fields tolerated). */
interface StopStdin {
  session_id?: string;
...
/** Injected dependencies — every side effect is a seam. */
export interface StopHookDeps {
  ...
  readonly now?: () => number;
  /** The poll tick sleeper (defaults to a real setTimeout wait). */
 ...
 */
export async function runStopHook(deps: StopHookDeps): Promise<void> {
  try {
    ...
    for (;;) {
      ...
      if (tips.length > 0) {
        ...
        for (const rest of tips.slice(1)) store.writeMailbox(payload.sessionId, rest);
        emitClaimed(tips[0], turnId, out);
        return;
      }
      // Judge finished and chose SILENCE (the majority case) → exit, near-zero stall.
      if (store.wasTurnJudged(payload.sessionId, turnId)) return;
      if (now() >= deadline) return; // cap: judge slow/crashed → labeled backstop next turn.
...
function parseStdin(stdin: string): { sessionId: string; cwd: string } | null {
  let raw: StopStdin;
  try {
    raw = JSON.parse(stdin) as StopStdin;
  } catch {
    return null;
  }
 ...
 */
function emitClaimed(tip: Tip, currentTurnId: string, out: NodeJS.WriteStream): void {
  const isLate = typeof tip.turnId === 'string' && tip.turnId.length > 0 && tip.turnId !== currentTurnId;
  const message =
    isLate && typeof tip.prompt === 'string' && tip.prompt.length > 0
