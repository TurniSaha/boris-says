 ...
 */
const SQL_DESTRUCTIVE_RES: readonly RegExp[] = [
...
const PROD_DATA_TARGET_RE =
  /\b(prod|production)\b/i;
const PERSISTENT_DATA_RE =
  /\b(table|tables|database|databases|\w*_?db|schema|rows?|records?|users?\s+table|migration)\b/i;
 ...
 */
export function namesDestructiveDataOp(prompt: string): boolean {
  const text = String(prompt);
  if (SQL_DESTRUCTIVE_RES.some((re) => re.test(text))) return true;
