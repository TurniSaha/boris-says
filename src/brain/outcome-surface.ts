...
export function surfaceOutcomeRecap(
  ...
  try {
    ...
    if (s.tourShown !== true && !stateShowsEngagement(s)) return false;
    ...
    if (!store.markOutcomeRecapShownIfFirst(sessionId)) return false;
    if (projectKey === '') return false; // unknown project → never surface (fail-safe).
