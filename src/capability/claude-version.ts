...
/** The minimal spawnSync surface this probe needs (DI'd for tests). */
type SpawnSyncFn = (
  command: string,
...
export interface ClaudeCliVersionOptions {
 ...
 */
export function claudeCliVersion(options: ClaudeCliVersionOptions = {}): string | null {
  const spawnSync = options.spawnSync ?? (nodeSpawnSync as unknown as SpawnSyncFn);
  try {
 ...
 */
function parseLeadingSemver(text: string): string | null {
