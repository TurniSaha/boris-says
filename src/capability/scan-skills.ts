 ...
 */
export const CURATED_SKILLS: readonly string[] = [
  'grill-me',
  'plan-optimizer',
  'tdd-workflow',
  'verification-loop',
...
export async function scanInstalledSkills(
  options: ScanInstalledSkillsOptions = {},
): Promise<string[]> {
  ...
  const readdir =
    options.readdir ??
    (fsReaddir as unknown as NonNullable<ScanInstalledSkillsOptions['readdir']>);
  const readFile =
    options.readFile ??
    (fsReadFile as unknown as NonNullable<ScanInstalledSkillsOptions['readFile']>);
 ...
 */
async function safeReaddirDirs(
  dir: string,
  deps: {
    fileExists: (path: string) => boolean;
    readdir: NonNullable<ScanInstalledSkillsOptions['readdir']>;
  },
): Promise<string[]> {
 ...
 * directory name.
 */
async function skillIdFor(
  skillPath: string,
  dirName: string,
  deps: { readFile: NonNullable<ScanInstalledSkillsOptions['readFile']> },
): Promise<string> {
  try {
...
function posixOrWin32Join(skillPath: string, leaf: string): string {
