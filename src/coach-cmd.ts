...
import {
  writeDraft as writeDraftFile,
  installInstructions,
  type DraftWriteResult,
} from './habit/draft-writer.js';

/** Env subset the backend-name resolver reads (mirrors BackendEnv, §6.3). */
interface CoachCmdEnv extends ConfigEnv {
  PROMPT_COACH_USE_API?: string | undefined;
  ANTHROPIC_API_KEY?: string | undefined;
  /** G-M4b: the index auto-refresh kill switch (status reports on/off). */
  PROMPT_COACH_NO_INDEX_REFRESH?: string | undefined;
}

/** The three names `/coach status` reports for the active backend (§6.3). */
export type BackendName = 'api-metered' | 'cli' | 'null';
...
export function resolveBackendName(
  ...
  if (useApi && typeof apiKey === 'string' && apiKey.length > 0) return 'api-metered';
  if (claudeOnPath()) return 'cli';
  return 'null';
}

/** Human-facing label for a resolved backend name (the `null` token reads as a bug otherwise). */
export function describeBackend(name: BackendName): string {
  switch (name) {
    case 'api-metered':
...
function fmtCooldownRemaining(lastAt: number | null, windowMs: number, now: number): string {
...
/** One labeled feedback anchor (a rated real fire) for the offline eval corpus (F-FEEDBACK). */
export interface FeedbackAnchor {
  readonly lever: string;
  readonly prompt: string;
  readonly sessionId: string;
  readonly rating: 'good' | 'bad';
  /** A 👎 means the fire was wrong → the gold label for this moment is SILENT (and vice-versa). */
  readonly goldVerdict: 'NUDGE' | 'SILENT';
  readonly at: number;
}

export interface CoachCmdDeps {
  store: Store;
  ...
  /**
   * F-FEEDBACK: append a labeled anchor to the local feedback corpus
   * (~/.claude/prompt-coach/feedback-anchors.jsonl). Injected so tests capture instead of
   * writing a real file; the default appends a JSONL line. Never throws.
   */
  recordFeedbackAnchor?: (anchor: FeedbackAnchor) => void;
  /**
   * W2-LEVEL1: read the raw feedback-anchors.jsonl text (for `/coach status` to show how many
   * taste examples are currently active). Injected so tests don't touch a real file. Default
   * reads `${baseDir}/feedback-anchors.jsonl` (never throws → '').
   */
  readFeedbackAnchorsText?: () => string;
  /**
   * M3: the home dir the skill review file goes under (default os.homedir()).
   * Injected so tests write under a tmp home, never the real ~/.claude.
   */
  homeDir?: string;
  /**
   * M3: injected review-file writer for `/coach build`. Default writes via
   * habit/draft-writer.js under homeDir + resolveBaseDir(env). Never throws.
   */
  writeDraft?: (draft: PatternDraft, habitKey: string) => DraftWriteResult;
  /**
   * M4: external-skill index loader for `/coach find` + the status line. Default is the
   * real loadSkillIndex (module-relative data/skill-index.json). null = index missing /
   * malformed → friendly one-liners, never a throw. `find` works on ANY loadable index
   * (no freshness gate — only the judge hot path is age-gated).
   */
  loadSkillIndex?: () => SkillIndex | null;
  /**
   * G-M4b: provenance-aware loader seam (which copy — runtime vs committed — was
   * chosen). Default is the real prefer-runtime loader over resolveBaseDir(env).
   * When only `loadSkillIndex` is injected (older tests), provenance is treated as
   * 'committed' (never labeled runtime).
   */
  loadSkillIndexWithProvenance?: () => LoadedSkillIndex | null;
  /**
   * M4: the dev's installed skill ids for `find`'s installed-catalog-wins exclusion.
   * Default is a lightweight best-effort SYNC directory scan; any error degrades to
   * NO exclusions (find still answers).
   */
  installedSkillIds?: () => readonly string[];
}

/** The three feedback subcommand aliases the owner can type. */
const GOOD_ALIASES: ReadonlySet<string> = new Set(['👍', 'up', 'good', '+1', 'yes']);
const BAD_ALIASES: ReadonlySet<string> = new Set(['👎', 'down', 'bad', '-1', 'no']);

/** Human label for a draft kind in `/coach build` output (mirrors the tip). */
const DRAFT_KIND_LABEL: Record<PatternDraft['kind'], string> = {
  skill: 'skill',
 ...
 */
export function runCoachCmd(subcommand: string | undefined, deps: CoachCmdDeps, extraArg?: string): void {
  ...
  const loadIndexLoaded = (): LoadedSkillIndex | null => {
    if (deps.loadSkillIndexWithProvenance) return deps.loadSkillIndexWithProvenance();
  ...
  switch (sub) {
    ...
    case 'build': {
      ...
      const writer =
        deps.writeDraft ??
        ((d: PatternDraft, k: string) => writeDraftFile(d, k, deps.homeDir ?? homedir(), resolveBaseDir(env)));
      ...
      out(`  ${result.path}`);
      for (const line of installInstructions(pattern.draft, result.path)) out(`  ${line}`);
    ...
    case 'find': {
      ...
      for (const h of hits) {
        ...
        const suffix =
          h.repoStars !== null ? ` (${trustLabel} · ★ ${h.repoStars})` : ` (${trustLabel})`;
    ...
    case 'status': {
      ...
      if (loadedIndex !== null) {
        ...
        out(
          `external index: ${loadedIndex.index.entries.length} entries (${officialCount} official, ` +
      ...
      const refreshOff =
        typeof env.PROMPT_COACH_NO_INDEX_REFRESH === 'string' &&
      ...
      if (watch.closedAt === null) {
        out(
          `critique mode: watching (${watch.promptsObserved}/${WATCH_MIN_PROMPTS} prompts, ` +
      ...
      if (watch.withheldCount > 0) {
        ...
        for (const w of recent) out(`  ${w.lever}: "${w.tip.slice(0, 100)}"`);
      }
      return;
    }
    default: {
      ...
      if (GOOD_ALIASES.has(sub) || BAD_ALIASES.has(sub)) {
        const rating: 'good' | 'bad' = GOOD_ALIASES.has(sub) ? 'good' : 'bad';
        ...
        try { deps.recordFeedbackAnchor?.(anchor); } catch { /* corpus append never blocks the rating */ }
        const delta = store.floorDeltaForLever(rated.tip.lever);
        const effect =
          delta > 0 ? ` — "${rated.tip.lever}" will fire less` : delta < 0 ? ` — "${rated.tip.lever}" will fire more` : ' (more ratings needed before it adapts)';
 ...
 */
function defaultInstalledSkillIds(deps: CoachCmdDeps): () => readonly string[] {
  return () => {
 ...
 */
export function parseCoachArgs(rest: readonly string[]): { subcommand: string | undefined; extra: string } {
...
export function main(argv: string[], env: CoachCmdEnv = process.env): void {
  try {
