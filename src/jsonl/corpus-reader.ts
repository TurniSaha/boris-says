...
export interface CorpusPrompt {
  ...
  /** Session id derived from the `.jsonl` filename basename (minus extension). */
  readonly sessionId: string;
  /** The project dir name the session file lives under. */
  readonly project: string;
  /** Epoch ms from the line's timestamp, or 0 when absent/unparseable. */
  readonly ts: number;
}

export interface CorpusReadOptions {
  /** Base projects dir; defaults to `~/.claude/projects`. */
  readonly projectsDir?: string;
  /**
   * Only return prompts with `ts > sinceWatermark` (epoch ms). The watermark lets
   * the miner read just what is new since its last run. Defaults to 0 (everything).
   */
  readonly sinceWatermark?: number;
  /** Cap on the number of session files read (most-recent by mtime). Default 200. */
 ...
 */
export function readCorpusTypedPrompts(options: CorpusReadOptions = {}): readonly CorpusPrompt[] {
  ...
  for (const file of bounded) {
    ...
    for (const line of raw.split('\n')) {
      ...
      const ts = event.ts ?? 0;
      if (ts <= sinceWatermark && sinceWatermark > 0) continue;
      prompts.push({
        text: event.text,
...
function collectSessionFiles(projectsDir: string): readonly SessionFile[] {
