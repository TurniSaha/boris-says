...
/** A genuine human-typed prompt extracted from one JSONL line. */
export interface TypedPromptEvent {
  /** The prompt text (string content, or the joined text blocks of an array). */
  readonly text: string;
  /** Session id, when the line carries one (top-level `sessionId`). */
  readonly sessionId?: string;
  /** Epoch ms parsed from the line's `timestamp`, when present and parseable. */
  readonly ts?: number;
}
 ...
 */
export function parseTypedPromptLine(line: string): TypedPromptEvent | null {
  let parsed: unknown;
  try {
  ...
  if (o.promptSource !== 'typed') return null;
  ...
  const event: TypedPromptEvent = { text };
 ...
 */
export function promptHasAttachedImage(prompt: string): boolean {
