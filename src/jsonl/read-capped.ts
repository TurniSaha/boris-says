 ...
 */
export function readFileCapped(path: string, maxBytes: number = MAX_JSONL_BYTES): string | null {
  let size: number;
  try {
  ...
  if (size > maxBytes) return null; // oversized → skip rather than balloon memory
  try {
