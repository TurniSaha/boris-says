...
const PRUNE_RECAP_CLAUSE = ' That change is large and uncommitted — worth a skim for dead code or one-off abstractions before building on it.';
const NO_TEST = {
...
function pytestSummary(text) {
    for (const line of text.split('\n')) {
        if (!/={3,}/.test(line) || !/\bpassed\b/.test(line))
            continue;
...
function cargoSummary(text) {
    for (const line of text.split('\n')) {
        const m = /test result:\s+\w+\.\s+(\d+)\s+passed;\s+(\d+)\s+failed/.exec(line);
        if (m)
            return { passed: Number(m[1]), failed: Number(m[2]), line: line.trim() };
    }
    return null;
}
const RUNNERS = [
 ...
 */
export function testSignal(events) {
    let latest = null;
...
export function commitSignal(events) {
    let latest = null;
    for (const e of events) {
...
export function changeSizeSignal(events) {
    let added = 0;
    let removed = 0;
    const files = new Set();
    let any = false;
    for (const e of events) {
        if (e.toolName !== 'Edit' && e.toolName !== 'Write' && e.toolName !== 'NotebookEdit')
            continue;
        if (e.patch !== null) {
            added += e.patch.added;
 ...
 */
function testCountUnit(runner, count) {
...
export function renderOutcomeLine(report) {
    ...
    if (report.tests.provenance === 'measured' && report.tests.ran) {
        if (report.tests.passed !== null) {
            const failClause = report.tests.failed !== null && report.tests.failed > 0 ? `, ${report.tests.failed} failed` : '';
        ...
        if (report.tests.coverage !== null)
            parts.push(`${report.tests.coverage}% coverage`);
    }
    else {
    ...
    const largeUncommitted = report.changeSize.provenance === 'measured' &&
