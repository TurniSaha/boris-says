 ...
 */
import { sanitizeExternalText } from '../capability/skill-index.js';
/**
 ...
 */
export const INTERRUPT_ELIGIBLE_PHASES = new Set([
    'new-task',
    'escalation',
    'ambiguous',
]);
...
export const RUBRIC_DIMENSIONS = [
    ...
    { id: 'scope_boundaries', label: 'Scope boundaries', probe: 'Is the change bounded, or open-ended/sprawling?' },
    {
        ...
        probe: 'Does the prompt (PLUS the recent transcript) give the agent enough to act? Terse is FINE when the context is anchored in recent turns OR when the prompt points at a fetchable external artifact (ticket/file/doc/URL/prior decision) the agent can open itself.',
    },
    ...
    { id: 'verification_path', label: 'Verification path', probe: 'Is there a way to verify (a test, a check) named or implied?' },
    {
        ...
        probe: 'Is the coding effort right for the task size? On an xhigh-capable model (Opus 4.8/4.7, Fable 5, Sonnet 5, Mythos 5), xhigh is the recommended coding effort for a gnarly task (default high). Flag xhigh/max effort on a one-liner, or default-high on a gnarly migration.',
    },
...
export function resolveJudgeModel(env = {}) {
 ...
 */
export const PROMPT_COACH_SKILL = {
...
export function buildProspectorUser(verbatim, transcript) {
    return [
 ...
 */
export function renderExternalSection(candidates) {
 ...
 */
export function buildJudgeUser(verbatim, transcript, rollingSummary, catalog, availableCapabilities, tasteSection = '', externalSection = '') {
