 ...
 */
const READ_ONLY_LEAD_RE = /^(check|look( at| into| through)?|read|show( me)?|list|find|search|grep|explain|describe|summarize|investigate|inspect|trace|compare|count|tell me|walk me through|why|what|where|when|which|who|how|is|are|does|do|did|was|were|can you (explain|tell|show|describe)|could you (explain|tell|show|describe))\b/;
/**
 ...
 */
const NOUN_POSITION_LOOKBEHIND = '(?<!\\b(?:the|a|an|this|that|these|those|your|my|our|its|their)\\s)';
const CHANGE_DIRECTED_RES = CHANGE_DIRECTED_TOKENS.map((tok) => new RegExp(`${NOUN_POSITION_LOOKBEHIND}\\b${tok}\\b`));
/**
...
export function classifyPromptIntent(prompt) {
    ...
    if (folded.length === 0)
        return 'unknown';
    if (!READ_ONLY_LEAD_RE.test(folded))
        return 'unknown';
    if (CHANGE_DIRECTED_RES.some((re) => re.test(folded)))
        return 'unknown';
    return 'read_only';
}
