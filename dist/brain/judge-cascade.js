 ...
 */
import { buildJudgeUser, buildProspectorUser, renderExternalSection, } from './prompt-coach-skill.js';
...
import { resolveCapability, } from '../capability/catalog.js';
import { QUALITY_COOLDOWN_MS, adaptiveFloorDelta } from '../state/store.js';
...
const UNDEFINED_TASK_LEVERS = new Set(['goal_clarity', 'scope_boundaries']);
/** Levers where an expensive multi-agent capability is the wrong tool (§5.5.5c). */
const SCOPE_FIRST_LEVERS = new Set(['scope_boundaries', 'acceptance_criteria']);
/**
 ...
 */
const CHANGE_DIRECTED_LEVERS = new Set([
    'verification_path',
    'risk_awareness',
    'acceptance_criteria',
    'process_fit',
...
function costClauseFor(costClass) {
    if (costClass === 'expensive_multiagent')
        return EXPENSIVE_COST_CLAUSE;
    if (costClass === 'billed')
        return BILLED_COST_CLAUSE;
    return '';
}
/**
 ...
 */
function nudgeNamesUnavailableCapability(nudge, available) {
    ...
    for (const token of backtickTokens(nudge)) {
        ...
        if (resolved.capability === null)
            continue; // not a capability → benign prose.
...
export async function runQualityCascade(input) {
    ...
    const capabilities = input.capabilities.filter((c) => {
        if (midSession && c.appliesAt === 'launch')
            return false;
        ...
        const scoped = c.modelFamilies ?? (c.modelFamily !== undefined ? [c.modelFamily] : undefined);
    ...
    if (prospectorScore.failOpen)
        observe('prospector_fail_open', false);
 ...
 */
function pingFallback(pingPrefix) {
 ...
 */
export function localContextSuppresses(lever, _verdict, localContext, prompt = '') {
    // SUP-1 — plan-mode-on → process_fit silent.
    if (lever === 'process_fit') {
        if (localContext.mode === 'plan')
            return true;
        if (localContext.project?.planModeMandated === true)
            return true;
        return false;
    }
    ...
    if (lever === 'risk_awareness') {
        ...
        if (namesDestructiveDataOp(prompt))
            return false;
 ...
 */
function composeTip(verdict, lever, catalog, available, activeModel, externalCandidates = []) {
    ...
    const action = !undefinedTask && verdict.skill_fit.candidate_skill
    ...
    const externalHit = !undefinedTask && action.kind === 'none' && verdict.skill_fit.candidate_skill
        ? (externalCandidates.find((c) => fold(c.name) === fold(verdict.skill_fit.candidate_skill)) ?? null)
        : null;
    // 2. Resolve the recommended capability — ONLY if available to THIS dev.
    const capabilityPerson = {
        installedCommands: null,
        cliVersion: null,
        activeModel,
    };
    const resolved = !undefinedTask && verdict.capability_fit.candidate_capability
        ? resolveCapability(verdict.capability_fit.candidate_capability, capabilityPerson, available)
        : null;
    let fitCapability = resolved && resolved.available ? resolved.capability : null;
    ...
    const capabilityPayload = action.kind !== 'none' || externalHit !== null ? null : fitCapability;
    ...
    let message = costClause.length > 0
    ...
    if (externalHit !== null) {
        ...
        const stars = externalHit.repoStars !== null
            ? community
    ...
    if (nudgeNamesUnavailableCapability(message, available))
        return null;
...
function capabilityLine(c) {
    const cost = c.costClass === 'expensive_multiagent'
