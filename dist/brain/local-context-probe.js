 ...
 */
export function gatherLocalContext(deps) {
 ...
 */
function liftFromJsonl(transcriptPath, readFile) {
    ...
    try {
        ...
        for (const line of raw.split('\n')) {
            ...
            if (o === null)
                continue;
            const msg = o.message !== null && typeof o.message === 'object'
                ? o.message
                : null;
            ...
            if (typeof o.mode === 'string' && o.mode.length > 0)
                mode = o.mode;
            if (typeof o.permissionMode === 'string' && o.permissionMode.length > 0)
                mode = o.permissionMode;
            // effort = LAST effort field, top-level or on the message.
            if (typeof o.effort === 'string' && o.effort.length > 0)
                effort = o.effort;
            if (msg !== null && typeof msg.effort === 'string' && msg.effort.length > 0)
                effort = msg.effort;
        }
    }
    catch {
 ...
 */
function liftGit(cwd, runGit) {
    try {
        const branchOut = runGit(['rev-parse', '--abbrev-ref', 'HEAD'], cwd);
        if (branchOut === null)
            return null; // no repo / git missing → UNKNOWN.
        const branch = branchOut.trim();
        if (branch.length === 0)
            return null; // empty → UNKNOWN.
 ...
 */
function liftProject(cwd, homeDir, readFile) {
    let claudeMdPresent = false;
    let testCmdDocumented = null;
    let planModeMandated = null;
    let hooksConfigured = null;
    // CLAUDE.md.
    try {
        ...
        if (claudeMd !== null) {
            ...
            if (TEST_CMD_SIGNALS.some((sig) => folded.includes(sig)))
                testCmdDocumented = true;
            if (mentionsPlanMandate(folded))
                planModeMandated = true;
        }
    }
    catch {
        // leave defaults.
    ...
    for (const settingsPath of [join(cwd, '.claude', 'settings.json'), join(homeDir, '.claude', 'settings.json')]) {
        try {
            ...
            if (hooksConfigured !== true && hasNonEmptyHooks(o.hooks))
                hooksConfigured = true;
            // A settings-level plan-mode mandate flag is also evidence.
            if (planModeMandated !== true && settingsMandatesPlan(o))
                planModeMandated = true;
        }
        catch {
...
function hasNonEmptyHooks(hooks) {
...
export function modelStringToFamily(raw) {
