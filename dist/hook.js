...
import { resolveBaseDir, resolveJudgePath, isEnabled, projectKeyForCwd } from './config.js';
...
export function runHook(deps) {
    try {
        ...
        if (payload === null)
            return; // unparseable / missing prompt -> nothing to do.
...
function drain(store, sessionId, out) {
    ...
    for (const rest of tips.slice(1))
        store.writeMailbox(sessionId, rest);
    const message = typeof tip.prompt === 'string' && tip.prompt.length > 0
 ...
 */
function detach(store, payload, deps) {
