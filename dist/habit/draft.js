...
const DRAFT_KINDS = new Set(['skill', 'claude_md_rule', 'hook']);
/**
 ...
 */
export const DRAFT_SYSTEM = 'You turn ONE recurring developer habit (mined from their real typed prompts) into ONE draft ' +
...
export function renderDraftRequest(pattern) {
    const evidence = pattern.occurrences
        .map((o) => `- [session ${o.sessionId}] ${o.evidence}`)
        .join('\n');
    return (`habit_key: ${pattern.habit_key}\n` +
        `habit: ${pattern.habit}\n` +
        `fix: ${pattern.fix}\n` +
...
export function parseDraft(text) {
    ...
    const o = obj;
    const kind = o.kind;
    if (typeof kind !== 'string' || !DRAFT_KINDS.has(kind))
        return null;
    const name = o.name;
    ...
    if (content.trim().length === 0)
        return null;
    if (content.length > MAX_DRAFT_CONTENT_CHARS)
        return null;
    if (kind === 'skill' && !hasFrontmatter(content))
        return null;
    return { kind: kind, name, content };
}
/**
...
function contentTokens(content) {
    ...
    for (const tok of anchorTokens(content)) {
        if (tok.length < 4)
            continue; // drop glue words / markdown noise.
        if (SCAFFOLD_TOKENS.has(tok))
            continue;
        out.add(tok);
    }
    return out;
 ...
 */
export function isDraftGrounded(draft, pattern) {
    const tokens = contentTokens(draft.content);
    if (tokens.size === 0)
        return false; // nothing checkable → no draft (fail closed).
    ...
    for (const src of sources)
        for (const tok of anchorTokens(src))
            grounded.add(tok);
    let hits = 0;
