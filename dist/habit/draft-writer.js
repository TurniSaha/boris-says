 ...
 */
export function resolveDraftTarget(draft, habitKey, homeDir, baseDir) {
 ...
 */
export function writeDraft(draft, habitKey, homeDir, baseDir) {
    try {
        const path = resolveDraftTarget(draft, habitKey, homeDir, baseDir);
        if (path === null)
            return { error: `unsafe draft name "${draft.name}"` };
 ...
 */
export function installInstructions(draft, path) {
    switch (draft.kind) {
        case 'skill':
