...
function parsePattern(v) {
    ...
    if (habit_key.length === 0)
        return null;
    const match_phrases = Array.isArray(o.match_phrases)
        ? o.match_phrases.filter((p) => typeof p === 'string' && p.trim().length > 0)
        : [];
    const anchorSignature = Array.isArray(o.anchorSignature)
        ? o.anchorSignature.filter((t) => typeof t === 'string' && t.length > 0)
        : undefined;
    const occurrences = Array.isArray(o.occurrences)
        ? o.occurrences.map(parseOccurrence).filter((x) => x !== null)
        : [];
    return {
        habit_key,
        match_phrases,
 ...
 */
export function parseMinerPatterns(text) {
    const arr = extractArray(text);
    if (!Array.isArray(arr))
        return [];
    return arr.map(parsePattern).filter((p) => p !== null);
}
