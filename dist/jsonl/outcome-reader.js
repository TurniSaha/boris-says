...
function patchCounts(structuredPatch) {
    ...
    for (const hunk of structuredPatch) {
        ...
        if (h === null || !Array.isArray(h.lines))
            continue;
 ...
 */
export function scanToolEvents(jsonlText) {
    ...
    for (const line of jsonlText.split('\n')) {
        ...
        if (o.type === 'assistant' && msg !== null && Array.isArray(msg.content)) {
            for (const block of msg.content) {
                ...
                if (b === null || b.type !== 'tool_use' || typeof b.id !== 'string' || typeof b.name !== 'string')
                    continue;
        ...
        if (o.type === 'user' && msg !== null && Array.isArray(msg.content)) {
            ...
            for (const block of msg.content) {
                const b = asObject(block);
                if (b === null || b.type !== 'tool_result' || typeof b.tool_use_id !== 'string')
                    continue;
                const use = pending.get(b.tool_use_id);
                if (use === undefined)
                    continue; // a result with no captured tool_use — skip.
