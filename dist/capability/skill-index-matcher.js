...
function hasNearMatch(token, pool) {
    for (const p of pool)
        if (nearMatch(token, p))
            return true;
    return false;
}
 ...
 */
export function matchExternalSkills(prompt, index, excludedNames, k = MAX_CANDIDATES, opts = {}) {
    ...
    if (promptTokens.size === 0 && promptPhrase.trim().length === 0)
        return [];
    const scored = [];
    for (const entry of index.entries) {
        if (excludedNames.has(fold(entry.name)))
            continue; // installed-catalog-wins.
        ...
        const exactNameMatch = (entry.trust === 'official' || userInitiated) &&
            nameTokens.length >= 2 &&
            namePhrase.length >= 3 &&
        ...
        const scoreFloor = (nameHit ? SCORE_FLOOR : KEYWORD_ONLY_SCORE_FLOOR) +
