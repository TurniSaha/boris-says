...
const GRADE_ALLOWLIST = new Set(['A', 'B']);
 ...
 */
export function repoFloorsOk(meta, now) {
    if (typeof meta.stars !== 'number' || meta.stars < MIN_STARS)
        return false;
 ...
 */
export function isCleanCommunityText(s) {
    if (typeof s !== 'string')
        return false;
    if (ANY_CTRL_RE.test(s))
        return false;
    if (ZERO_WIDTH_BIDI_RE.test(s))
        return false;
    if (BASE64_RUN_RE.test(s))
        return false;
    if (INJECTION_PHRASES_RE.test(s))
        return false;
    if (s.includes('<') || s.includes('>'))
        return false;
    if (BACKTICK_SHELL_RE.test(s))
        return false;
 ...
 */
export function curateCommunityEntries(rawMarketplaceJson, officialFoldedNames, _now) {
    if (rawMarketplaceJson === null || typeof rawMarketplaceJson !== 'object')
        return [];
    const plugins = rawMarketplaceJson.plugins;
    if (!Array.isArray(plugins))
        return [];
    const officialAlnum = new Set();
    for (const n of officialFoldedNames)
        officialAlnum.add(alnum(n));
    const ranked = [];
    ...
    ranked.sort((a, b) => {
        ...
        if (a.selfAuthored !== b.selfAuthored)
            return a.selfAuthored ? 1 : -1;
    ...
    for (const { entry } of ranked) {
        ...
        seen.add(key);
        out.push(entry);
        if (out.length >= COMMUNITY_MAX_ENTRIES)
            break;
    }
    return out;
}
...
function curateOne(item, officialFoldedNames, officialAlnum) {
    ...
    if (typeof p.name !== 'string' || !NAME_RE.test(p.name))
        return null;
    const name = p.name;
    if (officialFoldedNames.has(fold(name)) || officialAlnum.has(alnum(name)))
        return null;
    // Verification floor: grade A/B AND score ≥ 80 (absent block → reject).
    const ver = p.verification;
    if (ver === null || typeof ver !== 'object')
        return null;
    const grade = ver.grade;
    const score = ver.score;
    if (typeof grade !== 'string' || !GRADE_ALLOWLIST.has(grade))
        return null;
    if (typeof score !== 'number' || score < MIN_VERIFICATION_SCORE)
        return null;
    // Description: lint the RAW text (reject, never truncate), then window the sanitized.
    if (typeof p.description !== 'string' || !isCleanCommunityText(p.description))
        return null;
    ...
    const rawKeywords = Array.isArray(p.keywords)
        ? p.keywords.filter((k) => typeof k === 'string').slice(0, KEYWORDS_KEPT)
        : [];
    const keywords = [];
    for (const kw of rawKeywords) {
        if (!isCleanCommunityText(kw))
            return null;
        keywords.push(sanitizeCommunityText(kw));
    }
    // Category: optional; a dirty category is NULLED (it never rides a tip verbatim).
    const category = typeof p.category === 'string' && isCleanCommunityText(p.category)
        ? sanitizeCommunityText(p.category)
        : null;
    const author = p.author !== null && typeof p.author === 'object'
        ? p.author.name
        : null;
