 ...
 */
export async function scanInstalledCommands(options = {}) {
    ...
    const deps = { fileExists, readdir, join: j };
    ...
    for (const root of commandRoots) {
        ...
        if (containsExcludedSegment(root))
            continue;
 ...
 */
async function walkCommands(dir, ids, deps, depth) {
    ...
    for (const entry of entries) {
        if (entry.isDirectory()) {
            ...
            if (id.length > 0)
                ids.add(id); // first-seen wins (Set is insertion-deduped).
        }
    }
}
/**
