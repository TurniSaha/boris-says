 ...
 */
export function createMergedSkillCatalog(installedSkills, curatedSkills = CURATED_SKILLS) {
    ...
    const installedByFold = new Map();
    for (const id of installedSkills) {
        const key = fold(id);
        if (key.length > 0 && !installedByFold.has(key))
            installedByFold.set(key, id);
    }
    const curatedByFold = new Map();
    for (const id of curatedSkills) {
        const key = fold(id);
        if (key.length > 0 && !curatedByFold.has(key))
            curatedByFold.set(key, id);
    }
    ...
    const allByFold = new Map();
    for (const [key, id] of curatedByFold)
        allByFold.set(key, id);
    for (const [key, id] of installedByFold)
        allByFold.set(key, id);
    const all = [...allByFold.values()].sort();
    function resolveAction(skillId) {
        const key = fold(skillId);
        if (key.length === 0)
            return { kind: 'none' };
        const installed = installedByFold.get(key);
        if (installed !== undefined)
            return { kind: 'run', skillId: installed };
        const curated = curatedByFold.get(key);
        if (curated !== undefined)
            return { kind: 'install_run', skillId: curated };
        return { kind: 'none' };
    }
