...
import { runQualityCascade } from './brain/judge-cascade.js';
...
import { gatherLocalContext, modelStringToFamily, } from './brain/local-context-probe.js';
...
import { CAPABILITY_CATALOG, resolveCapability, } from './capability/catalog.js';
import { loadSkillIndexPreferRuntime, isFresh, } from './capability/skill-index.js';
import { runIndexRefresh, } from './capability/index-refresh.js';
 ...
 */
async function runQualityStep(deps, payload, store, backend, now) {
    ...
    const activeModel = deps.activeModel ?? modelStringToFamily(localContext.activeModel);
    ...
    try {
        const index = (deps.loadSkillIndex ?? (() => loadSkillIndexPreferRuntime(resolveBaseDir(deps.env))))();
        if (index !== null && isFresh(index, now())) {
            const excludedNames = new Set();
            for (const id of catalog.all)
                excludedNames.add(foldName(id));
 ...
 */
function depositAnnounce(store, payload, promptsObserved) {
    store.writeMailbox(payload.session_id, {
        kind: 'quality',
 ...
 */
async function runMinerStep(deps, store, patternsStore, backend, now) {
    const state = store.getState();
    const corpusReader = deps.readCorpus ??
        ((since) => readCorpusTypedPrompts({ sinceWatermark: since }));
 ...
 */
async function runIndexRefreshStep(deps, store, now) {
    const state = store.getState();
...
async function defaultBackend(env) {
 ...
 */
function readFeedbackAnchorsText(baseDir) {
    try {
...
async function defaultCapabilities(activeModel) {
    ...
    for (const cap of CAPABILITY_CATALOG) {
        ...
        if (resolved.available && resolved.capability !== null)
            out.push(resolved.capability);
    }
    return out;
}
